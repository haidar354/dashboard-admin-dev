...
export const canViewNavMenuGroup = (item: NavGroup) => {
  const hasAnyVisibleChild = item.children.some(i => isCanAccessRoute(i.rules || []))
  ...
  return isCanAccessRoute(item.rules)
}
...
export const canNavigate = (to: RouteLocationNormalized): boolean => {
  ...
  if (!to.meta.rules)
    return true
  else if (Array.isArray(to.meta.rules))
    return to.meta.rules.some(route => ability.can(route.action, route.subject))
